...
def update_server_data(object_storage):
    ...
    if answer.status_code == 404:
        logger.error("Token invalid, got 404, resetting token...")
    ...
    if (server_version := answer.json().get('version')) != object_storage.version:
        logger.info("Updating server version `{}` -> `{}`".format(server_version, object_storage.version))
    ...
    if (server_serial_no := answer.json().get('serial_no')) != object_storage.serial_no:
        logger.info("Updating server serial no `{}` -> `{}`".format(server_serial_no, object_storage.serial_no))
    ...
    if update_speaker_data_body:
        ...
        if not answer.ok:
            logger.error("Error updating speaker data: status code `{}`, text `{}`".format(
...
def auth_gate(object_storage):
    ...
    if not object_storage.token:
        if object_storage.auth_code is None:
            logger.warning("auth code is None")

            object_storage.play_speech.play(
                "Устройство не настроено. Cгенерируйте и воспроизведите аудиокод в мобильном приложении.", cache=True
            )
            data = json.loads(get_ggwave_input())
            object_storage.auth_code = data.get('code')

        logger.info("Token does not exist, authentication")
